...
arma::mat Rcpp_calc_jacobian2(const arma::vec& eta){
	...
	for(i = 0; i < D; i++){
	for(j = 0; j < D; j++){
		out_jacob.at(i,j) = eta.at(i)*((i==j) - eta.at(j));
	}
	}
	return out_jacob;
}

double Rcpp_calc_maf(const double& purity,const arma::vec& q, 
	const double& mult,const double& tCN,const arma::vec& u){
	return purity * mult * arma::dot(q,u) / 
		(tCN * purity + 2.0 * (1.0 - purity));
}

double test_round(const double& aa,const arma::uword& digits){
...
// [[Rcpp::export]]
double Rcpp_ITH_obsLL_Z(const arma::mat& RD,arma::mat& ZZ,
	const arma::mat& uniq_CN_MA,const arma::mat& BB,
	...
	for(vv = 0; vv < BB.n_cols; vv++){
		maf = Rcpp_calc_maf(purity,
			qq,
			uniq_CN_MA.at(vv,3),
			uniq_CN_MA.at(vv,0),
			eS.row( uniq_CN_MA.at(vv,4)-1 ).t());
		ZZ.col(vv) = arma::log(BB.col(vv)) + std::log(pi.at(vv)) +
...
// [[Rcpp::export]]
double Rcpp_ITH_compLL(const arma::mat& RD,const arma::mat& ZZ,
	const arma::mat& uniq_CN_MA,const arma::vec& pi,const arma::mat& eS,
	const double& purity,const arma::vec& qq){
...
// [[Rcpp::export]]
arma::vec Rcpp_ITH_compGRAD(const arma::mat& RD,const arma::mat& ZZ,
	const arma::mat& uniq_CN_MA,const arma::mat& eS,const double& purity,
...
// [[Rcpp::export]]
void Rcpp_ITH_Mstep_BFGS(const arma::mat& RD,const arma::mat& ZZ,
	const arma::mat& uniq_CN_MA,const arma::vec& pi,const arma::mat& eS,
	const double& purity,const arma::vec& tCN,const arma::vec& params0,
	arma::vec& qq,const arma::uword& max_iter = 4e3,
	...
	double curr_LL = 0.0;
	arma::vec tmp_qq = arma::zeros<arma::vec>(num_params+1);
...
// [[Rcpp::export]]
Rcpp::List Rcpp_ITH_opt(const arma::mat& RD,const arma::mat& BB,
	const arma::umat& uniq_BB,const arma::mat& uniq_CN_MA,
	const arma::mat& eS,const double& purity,
	const arma::vec& tCN,const arma::vec& pi0,
	const arma::vec& unc_qq0,const bool& mstep = true,
	const arma::uword& max_iter = 4e3,
	const double& eps = 1e-6,const bool& show = true){
	
	arma::uword LOCI = RD.n_rows;
	arma::uword DD = uniq_CN_MA.n_rows;
	arma::uword num_uniq_BB = uniq_BB.n_rows;
	arma::uword num_ss = eS.n_cols;
	...
	while( iter < max_iter ){
		// E-step
		if(num_ss == 1){
			old_LL = Rcpp_ITH_obsLL_Z(RD,ZZ,uniq_CN_MA,
				BB,pi,eS,purity,tmp_qq);
		} else {
			old_LL = Rcpp_ITH_obsLL_Z(RD,ZZ,uniq_CN_MA,
				BB,pi,eS,purity,Rcpp_vartheta_to_qq(unc_qq));
		}
		...
		for(bb = 0; bb < num_uniq_BB; bb++){
			...
			pi(tmp_index) = colsum_ZZ(tmp_index) / arma::sum(colsum_ZZ(tmp_index));
		}
		...
		// Code to set some pi's to zero if nothing classified
		CLASS.fill(0.0);
		for(bb = 0; bb < LOCI; bb++){
		...
		for(bb = 0; bb < num_uniq_BB; bb++){
			if( arma::any(freq_class(arma::find(uniq_BB.row(bb).t() == 1)) == 0.0) ){
				arma::uvec one_index = arma::find(uniq_BB.row(bb).t() == 1);
				arma::vec tmp_prop = pi(one_index);
				tmp_prop(arma::find(freq_class(one_index) == 0)).fill(0.0);
	...
	arma::uword num_params = num_ss - 1;
	for(bb = 0; bb < num_uniq_BB; bb++){
		num_params += arma::sum(1*(uniq_BB.row(bb).t() % pi > 0.0)) - 1;
	}
	
	old_LL = test_round(old_LL,2);
